...
def index():
    ...
    if request.args.get("prefill"):
        prefill = {
            ...
            "renter_street": request.args.get("street", ""),
            "renter_city":   request.args.get("city", ""),
            "renter_state":  request.args.get("state", ""),
            "renter_zip":    request.args.get("zip", ""),
        }
...
def admin_set_delivery(booking_id):
    ...
    try:
        if status == "none":
            ...
            cur.execute("UPDATE bookings SET delivery_status='picked_up',picked_up_at=NOW() WHERE id=%s",(booking_id,))
            conn.commit()
            # Background thread — send email + SMS without blocking the redirect
            import threading as _tpu
            def _bg_set_pickup(b_):
                try: send_pickup_confirmation(b_)
                except Exception as e: log.error(f"set-delivery pickup notify error: {e}")
            _tpu.Thread(target=_bg_set_pickup, args=(b,), daemon=True).start()
    except Exception as e:
...
<div class="container">
  ...
  {past_html}
  <a href="{BASE_URL}/?prefill=1&name={_cur_name}&email={_cur_email}&phone={_cur_phone}&company={_cur_company}&street={_cur_street}&city={_cur_city}&state={_cur_state}&zip={_cur_zip}" style="display:block;width:100%;padding:.85rem;background:linear-gradient(135deg,#16a34a,#15803d);color:white;border:none;border-radius:12px;font-size:.95rem;font-weight:800;text-align:center;text-decoration:none;box-sizing:border-box;margin-bottom:1rem;box-shadow:0 3px 10px rgba(22,163,74,.3)">\n    \U0001f389 Start a New Booking\n  </a>\n\n  My Information</div>\n  <div style="background:white;border:1.5px solid #e5e7eb;border-radius:14px;padding:1.25rem;margin-bottom:1rem;box-shadow:0 1px 4px rgba(0,0,0,.05)">\n    <p style="font-size:.83rem;color:#6b7280;margin-bottom:.85rem">Update your personal info — changes save instantly and we're notified.</p>\n    <form method="POST" action="/my-orders/update-profile">\n      <label style="display:block;font-size:.78rem;font-weight:700;color:#374151;margin-bottom:.25rem">Full Name</label>\n      <input type="text" name="full_name" value="{_cur_name}" required style="width:100%;padding:.65rem .85rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:.9rem;margin-bottom:.75rem;outline:none;box-sizing:border-box;font-family:inherit">\n      <label style="display:block;font-size:.78rem;font-weight:700;color:#374151;margin-bottom:.25rem">Email Address</label>\n      <input type="email" name="email" value="{_cur_email}" required style="width:100%;padding:.65rem .85rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:.9rem;margin-bottom:.75rem;outline:none;box-sizing:border-box;font-family:inherit">\n      <label style="display:block;font-size:.78rem;font-weight:700;color:#374151;margin-bottom:.25rem">Phone Number</label>\n      <input type="tel" name="phone" value="{_cur_phone}" placeholder="(203) 555-0100" style="width:100%;padding:.65rem .85rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:.9rem;margin-bottom:.75rem;outline:none;box-sizing:border-box;font-family:inherit">\n      <label style="display:block;font-size:.78rem;font-weight:700;color:#374151;margin-bottom:.25rem">Street Address</label>\n      <input type="text" name="renter_street" value="{_cur_street}" placeholder="123 Main St" style="width:100%;padding:.65rem .85rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:.9rem;margin-bottom:.75rem;outline:none;box-sizing:border-box;font-family:inherit">\n      <div style="display:grid;grid-template-columns:1fr .55fr .55fr;gap:.5rem;margin-bottom:.85rem">\n        <div><label style="display:block;font-size:.75rem;font-weight:700;color:#374151;margin-bottom:.2rem">City</label><input type="text" name="renter_city" value="{_cur_city}" placeholder="Waterbury" style="width:100%;padding:.6rem .7rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:.9rem;outline:none;box-sizing:border-box;font-family:inherit"></div>\n        <div><label style="display:block;font-size:.75rem;font-weight:700;color:#374151;margin-bottom:.2rem">State</label><input type="text" name="renter_state" value="{_cur_state}" placeholder="CT" maxlength="2" style="width:100%;padding:.6rem .7rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:.9rem;outline:none;box-sizing:border-box;font-family:inherit"></div>\n        <div><label style="display:block;font-size:.75rem;font-weight:700;color:#374151;margin-bottom:.2rem">Zip</label><input type="text" name="renter_zip" value="{_cur_zip}" placeholder="06701" maxlength="10" style="width:100%;padding:.6rem .7rem;border:1.5px solid #d1d5db;border-radius:8px;font-size:.9rem;outline:none;box-sizing:border-box;font-family:inherit"></div>\n      </div>\n      <button type="submit" style="width:100%;padding:.75rem;background:#6366f1;color:white;border:none;border-radius:8px;font-size:.92rem;font-weight:700;cursor:pointer">💾 Save Changes</button>\n    </form>\n  </div>\n\n  <div class="section-label" style="margin-top:1.75rem">Send Us a Message</div>
  <div style="background:white;border:1.5px solid #e5e7eb;border-radius:14px;padding:1.25rem;margin-bottom:1rem;box-shadow:0 1px 4px rgba(0,0,0,.05)">
...
@app.route("/my-orders/update-profile", methods=["POST"])
def customer_portal_update_profile():
    email, redir = _portal_require_auth()
    if redir: return redir
    full_name     = (request.form.get("full_name") or "").strip()
    new_email     = (request.form.get("email") or "").strip().lower()
    phone         = (request.form.get("phone") or "").strip()
    renter_street = (request.form.get("renter_street") or "").strip()
    renter_city   = (request.form.get("renter_city") or "").strip()
    renter_state  = (request.form.get("renter_state") or "").strip()
    renter_zip    = (request.form.get("renter_zip") or "").strip()
    if not full_name:
        return redirect(url_for("customer_portal_home") + "?profile_err=1")
    try:
        conn = get_db()
        if conn:
            cur = conn.cursor()
            cur.execute(
                "UPDATE bookings SET full_name=%s, phone=%s, renter_street=%s, renter_city=%s, renter_state=%s, renter_zip=%s WHERE LOWER(TRIM(email))=%s",
                (full_name, phone, renter_street, renter_city, renter_state, renter_zip, email)
            )
            if new_email and new_email != email:
                cur.execute("UPDATE bookings SET email=%s WHERE LOWER(TRIM(email))=%s", (new_email, email))
                session["portal_email"] = new_email
            conn.commit(); cur.close(); conn.close()
    except Exception as e:
        log.error(f"portal update-profile error: {e}")
    try:
        send_sms(BUSINESS_PHONE or "2037517964",
            f"ℹ️ Profile update from {full_name} ({email}): "
            f"phone={phone or '—'}, address={renter_street}, {renter_city} {renter_state} {renter_zip}",
            _admin=True)
    except Exception as e:
        log.error(f"portal profile SMS error: {e}")
    return redirect(url_for("customer_portal_home") + "?profile_ok=1")
